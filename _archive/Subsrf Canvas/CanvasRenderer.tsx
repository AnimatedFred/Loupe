...
function NodeFill({ fills }: { fills: CanvasNode['fills'] }) {
  if (!fills?.length) return null
  const fill = fills[0]
  if (fill.type === 'SOLID' && fill.color) {
    return null // handled via style
  }
  ...
  return (
    ...
    >
      ...
      {effectiveNode.type === 'TEXT' && (
        isLoadingState
          ? <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 4, height: '1em', width: '70%', animation: 'pulse 1.5s infinite' }} />
      ...
      {/* Heatmap overlay — simplified gradient */}
      ...
      {handles.map(h => (
        <div
          ...
          style={{
            ...
            borderRadius: 2, cursor: getCursor(h), zIndex: 70,
            ...handlePositions[h],
          }}
          onMouseDown={e => {
            e.stopPropagation()
...
// ── Drawing overlay (for new shapes) ─────────────────────────────────────

function DrawingOverlay({ rect }: { rect: { x: number; y: number; w: number; h: number } | null }) {
  if (!rect) return null
  return (
    <div style={{
      position: 'absolute',
...
export function CanvasRenderer() {
  ...
  const handleNodeSelect = useCallback((id: string, multi: boolean) => {
    if (multi) {
      useCanvasStore.getState().addToSelection(id)
    } else {
      setSelectedNodes([id])
    }
    ...
    >
        ...
        {activePage.nodes.map(node => (
          ...
          >
            <CanvasNodeRenderer
              ...
              breakpointWidth={breakpointWidth}
            />
            {/* Selection handles for single selected node */}
      ...
      {sketchMode && (
        <SketchOverlay onDone={strokes => {
          ...
          if (strokes.length > 0) {
            // Would send to AI for interpretation
          }
        }} />
      )}
