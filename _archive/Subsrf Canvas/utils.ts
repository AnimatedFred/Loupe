...
// Convert Color to Figma-style 0-1 float JSON
export function colorToFigma(c: Color) {
  return { r: c.r, g: c.g, b: c.b, a: c.a }
}

// ── CIELAB / Delta E for perceptual color comparison ─────────────────────
...
export function findClosestToken(value: string, tokens: DesignToken[], category: string): DesignToken | null {
  ...
  // Exact match for non-color tokens
...
export function generateNodeCode(node: CanvasNode, tokens: DesignToken[], framework: 'react-tailwind' | 'react-css' | 'html-css' = 'react-tailwind'): GeneratedCode {
...
function buildTailwindClasses(node: CanvasNode, tokens: DesignToken[]): string {
  ...
  if (node.type === 'TEXT' && node.typography) {
    ...
    classes.push(fontSizeToTailwind(t.fontSize, tokens))
...
function buildCSSRules(node: CanvasNode, tokens: DesignToken[], selector: string): string {
  ...
  if (shadow) {
    const tokenName = null // would check shadow tokens
...
function fontSizeToTailwind(px: number, tokens: DesignToken[]): string {
  const map: Record<number, string> = { 12:'text-xs', 14:'text-sm', 16:'text-base', 18:'text-lg', 20:'text-xl', 24:'text-2xl', 30:'text-3xl', 36:'text-4xl', 48:'text-5xl', 60:'text-6xl' }
  return map[px] ?? `text-[${px}px]`
}

function fontWeightToTailwind(w: number): string {
  const map: Record<number, string> = { 100:'font-thin', 200:'font-extralight', 300:'font-light', 400:'font-normal', 500:'font-medium', 600:'font-semibold', 700:'font-bold', 800:'font-extrabold', 900:'font-black' }
  return map[w] ?? `font-[${w}]`
}
...
function runLinterOnNodes(nodes: CanvasNode[], tokens: DesignToken[], violations: ConstraintViolation[]) {
  for (const node of nodes) {
    ...
    if (fill?.type === 'SOLID' && fill.color) {
      ...
      if (!match) {
        violations.push({
          ...
          fix: 'Assign closest token or add to token set',
          autoFixable: false,
        })
      }
    }
    ...
    // Text contrast
    if (node.type === 'TEXT' && node.typography) {
      // Simplified — in real impl would find parent bg
      ...
      if (ratio < 4.5) {
        violations.push({
          ...
          fix: 'Lighten text color to improve contrast',
          autoFixable: false,
        })
      }
    }
    ...
    if (node.autoLayout) {
      const al = node.autoLayout
      const offGridValues = [al.gap, al.paddingTop, al.paddingRight, al.paddingBottom, al.paddingLeft].filter(v => v > 0 && !isOnGrid(v))
    ...
    if (node.type === 'IMAGE' && !node.characters) {
      violations.push({
        ...
        fix: 'Add description in the element properties',
        autoFixable: false,
      })
    }
    ...
    if (node.typography) {
      ...
      if (!match) {
        violations.push({
          ...
          fix: 'Use a token-defined font size',
          autoFixable: false,
        })
      }
    }
