var LOUPE_CONFIG = {
  ...
  async refresh() {
    const data = await chrome.storage.local.get(['mcp_endpoint', 'mcp_token', 'loupe_session']);
    if (data.mcp_endpoint) this.MCP_ENDPOINT = data.mcp_endpoint;
    if (data.mcp_token)    this.MCP_TOKEN    = data.mcp_token;
    if (data.loupe_session) {
      this.CURRENT_TIER = (data.loupe_session.tier || 'free').toUpperCase();
    }
    return this;
  },
  ...
  async authHeader() {
    ...
    return { 'Authorization': `Bearer ${loupe_session.accessToken}` };
  },
