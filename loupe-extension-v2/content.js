// Loupe Intelligence — Content Engine v2.8 (Professional Workflow Update)
(function() {
  if (window.__loupeRunning) {
    console.log('[Loupe] Already running, skipping re-init to prevent duplicate listeners.');
    return;
  }
  window.__loupeRunning = true;
  ...
  function updateToolbar() {
    ...
    toolbar.innerHTML = `
      ...
      <div class="uipb-toolbar-divider"></div>
      <button class="uipb-toolbar-btn" id="uipb-preview" ${highlightedElements.length === 0 ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>Preview AI</button>
