...
function renderOutput() {
  if (elements.length === 0) {
    outputEl.innerHTML = esc([
      ...
      '',
      'How to use:',
      '  1. Click the Loupe extension icon in your browser toolbar',
  ...
  if (mode === 'prompt') {
    outputLabel.textContent = 'loupe-brief.txt';
    outputEl.innerHTML = generatePromptHTML(elements, context);
  } else {
    outputLabel.textContent = 'loupe-export.css';
...
function promptHeader(els, ctx) {
  ...
  return [
    `Loupe UI Brief: ${pageTitle} | ${sitePretty}`,
    `URL: ${ctx.url || 'Unknown'}`,
    '',
    ...
    '',
    'Visual Fidelity: Match the source colors, spacing, and typography exactly as provided in the Loupe data. Do NOT change the aesthetic direction.',
    '',
...
function buildCSSLines(el, i) {
  ...
  const selector = cls ? `.${cls}` : `.loupe-element-${i + 1}`;
...
btnPush.addEventListener('click', async () => {
  ...
  try {
    ...
    await fetch('https://web-production-9cce.up.railway.app/api/ai/push', {
      method: 'POST',
